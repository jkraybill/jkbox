...
import { type Triplet } from './triplet-finder.js'
...
function extractSrtFromVideo(videoPath: string, outputSrtPath: string): void {
	...
	try {
		probeOutput = execSync(`ffmpeg -i "${videoPath}" 2>&1 | grep "Stream.*Subtitle"`, {
			encoding: 'utf-8'
		})
	} catch (error: unknown) {
	...
	try {
		execSync(`ffmpeg -y -i "${videoPath}" -map ${englishStreamIndex} "${outputSrtPath}" 2>&1`, {
			encoding: 'utf-8',
			stdio: 'pipe'
		})
		...
		const stderr = execError.stdout || execError.stderr || execError.message || 'Unknown error'
		...
		console.error(
			'Command:',
			`ffmpeg -y -i "${videoPath}" -map ${englishStreamIndex} "${outputSrtPath}"`
		)
		console.error('')
		console.error('FFmpeg output:')
		console.error(stderr)
...
async function selectAudioStream(videoPath: string): Promise<number | null> {
	...
	if (streams.length === 1) {
		console.log(
			`✓ Single audio stream found: ${streams[0].fullIndex} [${streams[0].language || '?'}] - ${streams[0].codec}, ${streams[0].channels}`
		)
		if (streams[0].title) {
	...
	const criterionStreams = streams.filter((s) => s.title?.toLowerCase().includes('criterion'))

	if (criterionStreams.length === 1) {
		const selected = criterionStreams[0]
		console.log(
			`✓ Auto-selected stream ${selected.fullIndex} [${selected.language || '?'}] - ${selected.codec}, ${selected.channels}`
		)
	...
	const originalStreams = streams.filter((s) => s.title?.toLowerCase().includes('original'))
	...
	streams.forEach((stream, idx) => {
		...
		console.log(
			`  ${idx + 1}. Stream ${stream.fullIndex} ${lang} - ${stream.codec}, ${stream.channels}${title}`
		)
	})
	...
	const autoSelectHeuristic = (): number => {
		...
		const criterionStream = streams.find((s) => s.title?.toLowerCase().includes('criterion'))
		if (criterionStream) {
			console.log(
		...
		const originalStream = streams.find((s) => s.title?.toLowerCase().includes('original'))
		if (originalStream) {
			console.log(
	...
	.action(async (srtFile: string, options: { verbose?: boolean; maxSequences?: string }) => {
		try {
				...
				for (let i = 1; i < entries.length; i++) {
					...
					for (let fillers = 0; fillers <= 2; fillers++) {
						...
						if (isValidFirstTriplet(entries, i, frame2Idx, frame3Idx)) {
							...
							console.log(
								`  Found valid first triplet at index ${i} with ${fillers} filler(s) (frames ${entries[i].index}, ${entries[frame2Idx].index}, ${entries[frame3Idx].index}), keyword: "${keyword}"`
							)
			...
			if (sequencesToExport.length < tripletSequences.length) {
				console.log(
					`Limiting output to ${maxSeq} sequence(s) (found ${tripletSequences.length} total)`
				)
			}
...
program
	...
	.action(async (srtFile: string) => {
		try {
			...
			exportTopTriplets(srtFile, judgments)
		} catch (error) {
			if (error instanceof Error) {
				console.error(`Error: ${error.message}`)
			} else {
...
program
	...
	.action(async (filenameNoExt: string, maxNStr: string) => {
		try {
			...
			if (sequencesToExport.length < tripletSequences.length) {
				console.log(
					`Limiting output to ${maxN} sequence(s) (found ${tripletSequences.length} total)`
				)
			}
			...
			exportTopTriplets(srtPath, judgments, videoPath, audioStreamIndex)
			...
			// Explicitly exit to prevent hanging (event loop might have pending operations)
			process.exit(0)
		} catch (error) {
			if (error instanceof Error) {
				console.error(`Error: ${error.message}`)
