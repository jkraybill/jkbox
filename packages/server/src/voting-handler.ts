...
export class VotingHandler {
	 ...
	 * Requires at least 2 total players (human or AI) to start a game
	 * At least 1 human player must be present
	 */
	private computeAllReady(): boolean {
		...
		const totalPlayers = this.playerIds.size
		...
		// Must have at least 2 total players (can be human or AI)
		if (totalPlayers < 2) {
			return false
		}
